...
`n8n`) usan `env_file: ../../stack.env` en su `docker-compose.yml`. Ese `stack.env`
...
a `wss://kamehousecr.ddns.net/portainer/api/websocket/exec?...`, que sí
respeta el prefijo `/portainer/`. **nginx no aplica el reemplazo automático
del prefijo de location cuando `proxy_pass` usa variables** — reenvía la
URI tal cual la mandó el cliente, con `/portainer/` incluido, y Portainer
responde 404 (router de Go) porque no reconoce esa ruta.

El `location ^~ /portainer/api/websocket/` debe usar el mismo patrón que el
`location ^~ /portainer/` general: `rewrite ^/portainer/(.*)$ /$1 break;`
antes de un `proxy_pass` sin URI. Si otra ruta de Portainer devuelve "404
page not found" (texto literal de Go, no la página 404 de nginx) detrás de
`/portainer/`, sospechar primero de este patrón antes que de autenticación
o red.
...
| juegos | `stacks/juegos/docker-compose.yml` | `BASE_DIR` (contenido del sitio, sincronizado a mano desde el proyecto `juegoscyberseguridad` — ver `stacks/juegos/README.md`). Sale por `/juegos/` vía `proxy` |
| n8n | `stacks/n8n/docker-compose.yml` | `BASE_DIR`, `N8N_ENCRYPTION_KEY`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB`. Sale por `n8nkamehousecr.ddns.net` (dominio propio, en `EXTRA_DOMAINS` de `proxy`). **Ver `env_file` arriba** (solo el servicio `n8n`). |
