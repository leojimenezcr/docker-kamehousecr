#!/usr/bin/env bash
#
# trigger-portainer-redeploy.sh — dispara el webhook de auto-redeploy de un
...
if [[ ! -f "$WEBHOOKS_FILE" ]]; then
  ...
  echo "Copiar scripts/webhooks.env.example a scripts/webhooks.env y completar las URLs reales (Portainer > Stack > Webhooks)." >&2
  exit 1
fi
...
# Normaliza "jellyfin" -> "WEBHOOK_JELLYFIN", "rasa-faq-demo" -> "WEBHOOK_RASA_FAQ_DEMO"
...
echo "Disparando redeploy de '${STACK_NAME}' via webhook..."
